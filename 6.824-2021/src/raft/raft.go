...
func (rf *Raft) persist() {
	DPrintVerbose("[%v]: STATE: %v", rf.me, rf.logs)
	w := new(bytes.Buffer)
...
func (rf *Raft) Start(command interface{}) (int, int, bool) {
	...
	rf.persist()
	DPrintf("[%v]: term %v Start %v", rf.me, term, newLog)
...
func Make(peers []*labrpc.ClientEnd, me int,
	...
	rf.logs = make([]Entry, 0, 1)
	rf.logs = append(rf.logs, Entry{-1, 0, 0})
...
func (rf *Raft) commits() string {
	nums := make([]string, rf.lastApplied, rf.lastApplied)
	for i := 0; i <= rf.lastApplied; i++ {
		nums = append(nums, fmt.Sprintf("%4d", rf.logAt(i).Command))
	}
